...
function forms(formSelector, modalTimerId) {
  ...
  function bindpostData(form) {
    form.addEventListener('submit', event => {
      ...
      form.insertAdjacentElement('afterend', statusMsg); // const request = new XMLHttpRequest();
  ...
  fetch('http://localhost:3000/menu').then(data => data.json()).then(res => console.log(res)); // fetch('https://jsonplaceholder.typicode.com/posts',{
  //     method: "POST",
...
function modal(triggerSelector, modalSelector, modalTimerId) {
  ...
  const btns_modalOpen = document.querySelectorAll(triggerSelector),
        info_modal = document.querySelector(modalSelector); // info_Nmodal = document.querySelector('.modal__dialog');
  ...
  window.addEventListener('scroll', showModalByScroll); // btns_modalClose.addEventListener('click', closeModal);
...
function slider({
  ...
  const indicators = document.createElement('ol'),
        dots = []; // indicators.classList.add('carousel-indicators');
...
`;
  ...
  dots.forEach(dot => {
    dot.addEventListener('click', event => {
      ...
      slideIndex = slideTo; //offset = +width.slice(0, width.length - 2) * (slideTo - 1);
...
const getResource = async url => {
  const res = await fetch(url); //тк catch для fetch не отлавливает ошибку при неправильном запросе
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_3__.default)('.timer', '2021-08-25');
